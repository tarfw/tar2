...
// Delete a product and all associated items and cart entries (with cascading)
export const deleteProduct = async (productId: string) => {
  try {
    // Query all associated data
    ...
    // Build deletion transactions
    const transactions = [
      ...associatedItems.map(item => db.tx.items[item.id].delete()),
      ...associatedCartItems.map(cart => db.tx.cart[cart.id].delete()),
...
export const bulkDeleteProducts = async (productIds: string[]) => {
  try {
    // Query all associated data for each product
    ...
    // Build deletion transactions
    const transactions = [
      ...allAssociatedItems.map(item => db.tx.items[item.id].delete()),
      ...allAssociatedCartItems.map(cart => db.tx.cart[cart.id].delete()),
      ...productIds.map(id => db.tx.products[id].delete())
    ];
