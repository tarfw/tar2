          ...
          onPress: async () => {
            try {
              ...
              if (result.success) {
                ...
                setIsMultiSelectMode(false);
                setShowBottomDrawer(false);
                log.info('Products deleted successfully', 'ProductsScreen', { count: productIds.length });
              } else {
                Alert.alert('Error', result.error || 'Failed to delete products');
              }
            } catch (error) {
              trackError(error as Error, 'ProductsScreen', { operation: 'bulkDelete' });
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              if (result.success) {
                log.info('Product deleted successfully', 'ProductsScreen', { productId: product.id, productName });
              } else {
                Alert.alert('Error', result.error || 'Failed to delete product');
              }
            } catch (error) {
              trackError(error as Error, 'ProductsScreen', { operation: 'deleteProduct', productId: product.id });
