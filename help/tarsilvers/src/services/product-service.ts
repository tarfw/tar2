...
export class ProductService {
  ...
  // Delete a product and all associated items and cart entries
  async deleteProduct(productId: string): Promise<{ success: boolean; error?: string }> {
    try {
      log.info('🗑️ Starting product deletion with cascading', 'ProductService', { productId });

      // Step 1: Query all associated data
      log.info('📋 Querying associated data...', 'ProductService', { productId });
      ...
      const associatedItems = data.items || [];
      const associatedCartItems = data.cart || [];
      
      log.info('📊 Found associated data', 'ProductService', { 
        productId, 
        itemCount: associatedItems.length,
        cartItemCount: associatedCartItems.length,
        itemIds: associatedItems.map(item => item.id),
        cartItemIds: associatedCartItems.map(cart => cart.id)
      });

      // Step 2: Build deletion transactions
      const transactions = [];
      
      // Add item deletions
      associatedItems.forEach(item => {
        log.info('➕ Adding item deletion to transaction', 'ProductService', { itemId: item.id, sku: item.sku });
        transactions.push(db.tx.items[item.id].delete());
      });
      
      // Add cart deletions
      associatedCartItems.forEach(cart => {
        log.info('➕ Adding cart deletion to transaction', 'ProductService', { cartId: cart.id });
        transactions.push(db.tx.cart[cart.id].delete());
      });
      
      // Add product deletion
      log.info('➕ Adding product deletion to transaction', 'ProductService', { productId });
      transactions.push(db.tx.products[productId].delete());

      log.info('🔄 Executing transaction with cascading deletions', 'ProductService', { 
        productId,
        totalTransactions: transactions.length 
      });

      // Step 3: Execute all deletions in a single transaction
      await db.transact(transactions);

      log.info('✅ Product and associated data deleted successfully', 'ProductService', { 
        productId, 
        deletedItemsCount: associatedItems.length,
        deletedCartItemsCount: associatedCartItems.length,
        totalDeleted: transactions.length
      });
      
      return { success: true };
    } catch (error) {
      log.error('❌ Product deletion failed', 'ProductService', { 
        productId, 
  ...
  // Bulk delete products and all associated items and cart entries
  async bulkDeleteProducts(productIds: string[]): Promise<{ success: boolean; error?: string }> {
    try {
      log.info('🗑️ Starting bulk product deletion with cascading', 'ProductService', { productIds, count: productIds.length });

      // Step 1: Query all associated data for each product
      log.info('📋 Querying associated data for all products...', 'ProductService', { productIds });
      ...
      const allAssociatedItems = dataQueries.flatMap(d => d.items);
      const allAssociatedCartItems = dataQueries.flatMap(d => d.cart);
      
      log.info('📊 Found associated data for bulk deletion', 'ProductService', { 
        productCount: productIds.length,
        itemCount: allAssociatedItems.length,
        cartItemCount: allAssociatedCartItems.length,
        itemIds: allAssociatedItems.map(item => item.id),
        cartItemIds: allAssociatedCartItems.map(cart => cart.id)
      });
      ...
      // Add cart deletions
      allAssociatedCartItems.forEach(cart => {
        transactions.push(db.tx.cart[cart.id].delete());
      });
      
      // Add product deletions
      productIds.forEach(productId => {
        transactions.push(db.tx.products[productId].delete());
      });

      log.info('🔄 Executing bulk transaction with cascading deletions', 'ProductService', { 
        productCount: productIds.length,
        totalTransactions: transactions.length 
      });
      ...
      log.info('✅ Bulk products and associated data deleted successfully', 'ProductService', { 
        productCount: productIds.length,
        deletedItemsCount: allAssociatedItems.length,
        deletedCartItemsCount: allAssociatedCartItems.length,
        totalDeleted: transactions.length
      });
      
      return { success: true };
    } catch (error) {
      log.error('❌ Bulk product deletion failed', 'ProductService', { 
        productIds, 
        error: error instanceof Error ? error.message : 'Unknown error',
        stack: error instanceof Error ? error.stack : undefined
      });
