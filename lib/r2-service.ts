...
const trackError = (error: Error, context: string, metadata?: any) => {
...
class R2Service {
  ...
  async uploadFile(file: MediaFile, username: string = 'user', prefix?: string): Promise<UploadResult> {
    ...
    try {
      ...
      const basePath = prefix !== undefined ? `${username}/${prefix}` : username;
      const key = `${basePath}/${Date.now()}-${Math.random().toString(36).substring(2, 15)}.${file.name?.split('.').pop() || ''}`;
      console.log(`[R2Service] Uploading file: ${file.name} (${file.type}, ${file.size} bytes)`);

      // Read file content with improved error handling
      let body: Uint8Array;
      try {
        console.log(`[R2Service] Fetching file from URI: ${file.uri.substring(0, 100)}...`);
        ...
        console.error(`[R2Service] File read error for ${file.name}:`, fileReadError);
        return {
          success: false,
      ...
      console.log(`[R2Service] File read successfully, size: ${body.byteLength} bytes`);
      ...
      console.log(`[R2Service] Sending upload command to R2 bucket: ${r2Config.bucketName}, key: ${key}`);
      await this.client.send(command);
      console.log(`[R2Service] Upload successful for ${file.name}`);

      // Generate public URL
      const url = getPublicUrl(key);
      console.log(`[R2Service] Generated public URL: ${url}`);
      ...
      console.error(`[R2Service] Upload error for ${file.name}:`, error);
  ...
  async isFileReferenced(fileUrl: string, db: any): Promise<boolean> {
    try {
      ...
      console.error('Error checking file references', 'R2Service', { error });
  ...
  async getFileMetadata(key: string): Promise<{ size?: number; lastModified?: Date; contentType?: string } | null> {
    ...
    try {
      ...
      console.error(`Failed to get metadata for ${key}`, 'R2Service', { error });
      return null;
    }
  }
